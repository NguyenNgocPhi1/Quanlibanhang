(function ($) {
    ...
    HT.createVariantRow = (attributeItem, variantItem) => {
        ...
        $td = $('<td>').append(
            $('<span>').addClass('image img-cover').append(
                $('<img>').attr('src', 'backend/img/not-found.jpg').addClass('imageSrc')
            )
        )
        $row.append($td)
    ...
    HT.deleteVariantAlbum = function () {
        $(document).on('click', '.variant-delete-image', function () {
            const $btn = $(this)
            const $imageItem = $btn.closest('.ui-state-default')
            const $container = $btn.closest('.upload-variant-list')
            const $uploadButton = $container.siblings('.click-to-upload-variant')
        
            // Xoá phần tử
            $imageItem.remove()
        
            // Chờ DOM update xong mới kiểm tra (tránh bug hiển thị)
            setTimeout(() => {
                const remaining = $container.find('.ui-state-default').length
                if (remaining === 0) {
